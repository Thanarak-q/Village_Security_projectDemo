  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        if (!liffId) {
          setStep("error");
          setMsg("ไม่มี NEXT_PUBLIC_GUARD_LIFF_ID");
          return;
        }
        ...
        if (idToken) {
          setIdToken(idToken);
          try {
            console.log('🔍 Verifying guard with backend...');
            ...
            if (authResult.success && authResult.user && authResult.token) {
              ...
              console.log('✅ Guard found in database, showing success message');
              storeAuthData(authResult.user, authResult.token);
              setStep("ready");
              setMsg("เข้าสู่ระบบสำเร็จ! คุณสามารถปิดหน้านี้ได้แล้ว");
            } else if (authResult.expectedRole) {
              // User is already registered but using wrong LIFF app
              console.log('⚠️ User already registered as', authResult.expectedRole, 'but using guard LIFF');
              setStep("ready");
