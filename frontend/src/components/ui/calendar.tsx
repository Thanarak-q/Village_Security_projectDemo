/**
 * @file This file provides a highly customizable calendar component for date selection.
 *
 * Built on top of the `react-day-picker` library, this component offers a rich set of
 * features, including single date selection, range selection, and custom styling,
 * all integrated with Tailwind CSS for a consistent design.
 */
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(buttonVariants({ variant: buttonVariant }), "size-8 aria-disabled:opacity-50 p-0 select-none", defaultClassNames.button_previous),
        button_next: cn(buttonVariants({ variant: buttonVariant }), "size-8 aria-disabled:opacity-50 p-0 select-none", defaultClassNames.button_next),
      ...
      components={{
        ...
        WeekNumber: ({ children, ...props }) => <td {...props}><div className="flex size-8 items-center justify-center text-center">{children}</div></td>,
        ...components,
      }}
      {...props}
    />
...
/**
 * A button component representing a single day in the calendar.
 *
 * @param {React.ComponentProps<typeof DayButton>} props - The props for the component.
 * @returns {React.ReactElement} The day button component.
 */
function CalendarDayButton({ className, day, modifiers, ...props }: React.ComponentProps<typeof DayButton>): React.ReactElement {
  const defaultClassNames = getDefaultClassNames();
  ...
  return (
    <Button
      ...
      className={cn("data-[selected-single=true]:bg-primary data-[selected-single=true]:text-primary-foreground data-[range-middle=true]:bg-accent data-[range-middle=true]:text-accent-foreground data-[range-start=true]:bg-primary data-[range-start=true]:text-primary-foreground data-[range-end=true]:bg-primary data-[range-end=true]:text-primary-foreground group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-ring/50 dark:hover:text-accent-foreground flex aspect-square size-auto w-full min-w-8 flex-col gap-1 leading-none font-normal group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:ring-[3px] data-[range-end=true]:rounded-md data-[range-end=true]:rounded-r-md data-[range-middle=true]:rounded-none data-[range-start=true]:rounded-md data-[range-start=true]:rounded-l-md [&>span]:text-xs [&>span]:opacity-70", defaultClassNames.day, className)}
      {...props}
    />
  );
}
