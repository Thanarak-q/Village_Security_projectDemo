...
export const houseMemberRoutes = new Elysia({ prefix: "/api" })
  ...
  .get("/house-members/village/:village_key", async ({ params }) => {
    try {
      ...
      return { success: false, error: "Failed to fetch house members for village" };
    }
  })
  ...
  .get("/house-members/house/:house_id", async ({ params }) => {
    try {
      ...
      return { success: false, error: "Failed to fetch house members for house" };
    }
  })
  ...
  .get("/house-members/resident/:resident_id", async ({ params }) => {
    try {
      ...
      return { success: false, error: "Failed to fetch house members for resident" };
    }
  })
  
  // Get single house member by ID
  ...
  .post("/house-members", async ({ body }) => {
    try {
      ...
      if (!house_id || !resident_id) {
        return { 
          success: false, 
          error: "Missing required fields! house_id and resident_id are required." 
        };
      }
      ...
      const existingHouseMember = await db
        .select()
        .from(house_members)
        .where(eq(house_members.house_id, house_id) && eq(house_members.resident_id, resident_id));
