...
export const adminRoutes = new Elysia({ prefix: "/api" })
  ...
  /**
   * Get all admins.
   * @returns {Promise<Object>} A promise that resolves to an object containing the admins.
   */
  .get("/admins", async () => {
    try {
   ...
   * @param {Object} params - The parameters for the request.
   * @param {string} params.admin_id - The admin ID.
   * @returns {Promise<Object>} A promise that resolves to an object containing the admin.
   */
   ...
   * @returns {Promise<Object>} A promise that resolves to an object containing the new admin.
   */
  .post("/admins", async ({ body }) => {
    try {
      const { email, username, password_hash, phone, village_key, status } =
        body as {
          email: string;
          username: string;
      ...
      if (!email || !username || !password_hash || !phone || !village_key) {
        return {
          success: false,
          error:
            "Missing required fields! email, username, password_hash, phone, and village_key are required.",
        };
      }
   ...
   * @param {string} params.admin_id - The admin ID.
   * @param {Object} body - The body of the request.
   * @returns {Promise<Object>} A promise that resolves to an object containing the updated admin.
   */
   ...
   * @param {Object} params - The parameters for the request.
   * @param {string} params.village_key - The village key.
   * @returns {Promise<Object>} A promise that resolves to an object containing the admin count.
   */
