...
export const visitorRecordMonthlyRoutes = new Elysia({ prefix: "/api" })
  ...
  .get("/visitor-record-monthly", async ({ currentUser, query, request }: any) => {
    try {
      ...
      console.log("Monthly Records - Extracted village_id:", village_id);
      console.log("Monthly Records - Available village_ids:", village_ids);
      ...
      const result = await getMonthlyVisitorRecords([village_id], role);
      return {
        success: true,
        data: result,
